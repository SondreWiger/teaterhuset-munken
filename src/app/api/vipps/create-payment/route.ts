  ...
  try {
    ...
    if (
      ...
      return NextResponse.json(
        {
          error: "Vipps ikke konfigurert",
          message:
            "For å bruke Vipps, sett VIPPS_CLIENT_ID, VIPPS_CLIENT_SECRET og VIPPS_MERCHANT_SERIAL i .env.local",
        },
        { status: 501 }
      );
    }

    // Vipps API uses OAuth2 client credentials
    ...
    if (!tokenRes.ok) {
      ...
      return NextResponse.json(
        { error: "Kunne ikke autentisere med Vipps" },
        { status: 500 }
      );
    }
    ...
    const paymentRes = await fetch(
      "https://api.vipps.no/epayment/v1/payments",
      {
        ...
        body: JSON.stringify({
          ...
          paymentDescription: `Kjøp av video`,
